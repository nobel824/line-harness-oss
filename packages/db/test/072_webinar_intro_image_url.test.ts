...
describe('072_webinar_intro_image_url.sql', () => {
  test('intro_image_url は NULL 許容の TEXT 列として追加される', () => {
    const db = createDatabase();
    const column = db
      .prepare(`PRAGMA table_info(webinars)`)
      .all()
      .find((row) => (row as { name: string }).name === 'intro_image_url') as {
        type: string;
        notnull: number;
      } | undefined;
  ...
  test('intro_image_url を指定しない既存形式の INSERT は通り、intro_image_url は NULL になる', () => {
    const db = createDatabase();
    db.prepare(
    ...
    const row = db
      .prepare(`SELECT intro_image_url FROM webinars WHERE id = ?`)
      .get('webinar-1') as { intro_image_url: string | null };
    expect(row.intro_image_url).toBeNull();
  });
});
